...
func InitCLIConfig(flagPath string, loadDefaults bool) (*CLI, error) {
	...
	possibleConfigPaths := []string{
		flagPath,
		fmt.Sprintf("%s/%s", homeDir, ".basecoat.hcl"),
		fmt.Sprintf("%s/%s/%s", homeDir, ".config", "basecoat.hcl"),
	}
	...
	envPath := os.Getenv("BASECOAT_CLI_CONFIG_PATH")
	if envPath != "" {
		path = envPath
	}
	...
	err := configParser.Load(env.Provider("BASECOAT_CLI_", "__", func(s string) string {
		newStr := strings.TrimPrefix(s, "BASECOAT_CLI_")
...
func GetCLIEnvVars() []string {
	vars := getEnvVarsFromStruct("BASECOAT_CLI_", structs.Fields(CLI{}))
	sort.Strings(vars)
	return vars
}
