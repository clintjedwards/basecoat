...
func (bc *API) CreateAPIToken(context context.Context, request *api.CreateAPITokenRequest) (*api.CreateAPITokenResponse, error) {
	...
	if err != nil {
		...
		bc.log.Errorw("could not authenticate account", "error", err, "account", request.Id)
	...
	bc.log.Infow("api token created", "account", request.Id)
