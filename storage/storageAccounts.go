...
// UpdateAccount modifies a single account
func (db *BoltDB) UpdateAccount(id string, updatedAccount *api.Account) error {
	...
	err := db.store.Update(func(tx *bolt.Tx) error {
		...
		accountRaw, err := go_proto.Marshal(updatedAccount)
		if err != nil {
			return err
		}
...
// DisableAccount disables a login account by key
func (db *BoltDB) DisableAccount(id string) error {
	var storedAccount api.Account
	...
	err := db.store.Update(func(tx *bolt.Tx) error {
		...
		currentAccount := bucket.Get([]byte(id))
		if currentAccount == nil {
			return fmt.Errorf("could not get account: %s;%w", id, tkerrors.ErrEntityNotFound)
		}

		err := go_proto.Unmarshal(currentAccount, &storedAccount)
		if err != nil {
			return err
		}

		storedAccount.State = api.Account_DISABLED

		accountRaw, err := go_proto.Marshal(&storedAccount)
		if err != nil {
			return err
		}
